...
class ExplorarExercicioAdapter(private val context: Context): RecyclerView.Adapter<ExplorarExercicioAdapter.MinhaViewHolder>() {

    // Tem uma lista tipo nosso modelo de classes (utilizadores)
    private var lista_exercicio = mutableListOf<PublicarExercicio>()
    ...
    inner class MinhaViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindView(exercicio:PublicarExercicio){
            Glide.with(context).load(exercicio.imagem).into(itemView.imagem_exercicio_template)
            itemView.titulo_exercicio_template.text = exercicio.titulo
            itemView.descricao_exercicio_template.text = exercicio.descricao
            itemView.tema_exercicio_template.text = exercicio.tema
        }
    }
    ...
    override fun getItemCount(): Int {
        return if(lista_exercicio.size > 0) {
            lista_exercicio.size
        }else {
            0
        }
    }
    ...
    override fun onBindViewHolder(guardar_linha_customizada: MinhaViewHolder, posicao: Int) {
        val item_atual = lista_exercicio[posicao]
    ...
    fun setData(data_exercicio: MutableList<PublicarExercicio>, idutilizador: Int, fragment: String) {
        this.lista_exercicio = data_exercicio
